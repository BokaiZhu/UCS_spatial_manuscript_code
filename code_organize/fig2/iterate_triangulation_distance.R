...
iterate_triangulation_distance <- function(df_input, num_iterations = 1000, id, x_pos, y_pos, cell_type, region, num_cores = NULL, threshold_pix = 512){
  # Get unique regions
  ...
  output_results <- foreach(i = 1:length(unique_regions), .packages = c("deldir", "tidyverse"), .combine = "rbind", .options.snow = opts)%dopar%{
    ...
    for (permutations in 1:num_iterations){
      ...
      rdelaun_result <- rdelaun_result %>%
        ...
        dplyr::filter(distance <= threshold_pix) %>%
